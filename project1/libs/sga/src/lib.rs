...
pub struct Individual {
    pub genome: Vec<bool>, // For each index in the data, "is included in the knapsack" or not
        ...
        for (i, &in_sack) in self.genome.iter().enumerate() {
            if in_sack {
                ...
                if self.weight > *capacity {
                    ...
                    let penalty = PENALTY_K * d;
                    self.fitness_score = self.profit.saturating_sub(penalty);
                    //println!("Penalty! Weight was: {:?}", self.weight);
                } else {
...
fn tournament_selection(population: &Population, k: usize) -> &Individual {
    ...
    for _ in 1..k {
        ...
        if population.individuals[idx].fitness_score
            > population.individuals[best_index].fitness_score
        {
            best_index = idx;
        }
    }
...
pub fn sga(
    ...
    for _gen in 1..=generations {
        ...
        for (index, i) in population.iter_mut().enumerate() {
            ...
            if i.fitness_score > best_score && i.weight <= capacity {
        ...
        let mean_fit = if count == 0 {
            0.0
        } else {
            sum_fit as f64 / count as f64
        };
        ...
        for _sel in 0..pop_size / 2 {
            ...
            let (offspring1, offspring2) =
                single_point_crossover(parent1, parent2, crossover_point);

            // Mutate the offspring
            let (mutant1, mutant2) = (mutate(&offspring1, MUTATION_PROBABILITY), mutate(&offspring2, MUTATION_PROBABILITY));

            // Populate new_individuals with the (possibly mutated) mutants
            new_individuals.extend_from_slice(&[
                Individual::from_genome(mutant1),
                Individual::from_genome(mutant2),
            ]);
        }
    }
...
mod tests {
    ...
    pub fn test_individual_fitness() {
        ...
        assert_eq!(individual1.fitness_score, 0);
